...
- The advanced node no longer has a `参考图像` input.
- `输入潜空间` is required.
- The node first samples `输入潜空间` once at its original latent size with the same KSampler Advanced step settings.
...
Connect `输入潜空间`; the node first runs the selected advanced segment at the latent's original size, decodes that result into the internal reference image, then performs the high-resolution reference redraw.
