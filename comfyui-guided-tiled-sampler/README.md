...
- If both `参考图像` and `输入潜空间` are connected, the node uses `输入潜空间` as the initial latent canvas and scales it directly to the target latent size, while `参考图像` remains the reference source.
- If `参考图像` is not connected, `输入潜空间` is decoded with the VAE and used as the fallback reference image; in this fallback mode it must be a clean latent.
- A leftover noisy latent can be tried only when `参考图像` is also connected and remains the reference source. Do not use noisy latent as the only reference source, because decoding it produces colored noise.
...
1. First segment: connect a clean reference image to `参考图像`, set `加噪 = 启用`, `起始步 = 0`, `结束步 = 3`, `保留剩余噪声 = 启用`.
2. Next segment: connect the previous segment's latent to `输入潜空间`, keep the same `参考图像`, `总步数` / `采样器` / `调度器` / prompt, set `加噪 = 禁用`, `起始步 = 3`, `结束步 = 总步数`, `保留剩余噪声 = 禁用`.
