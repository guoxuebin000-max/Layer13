...
Reference priority and latent input:

- If `参考图像` is connected, it is always the reference source and controls the target aspect ratio.
- If `参考图像` is not connected, `输入潜空间` is first sampled once at its original latent size with the same KSampler Advanced step settings, then decoded into the fallback reference image, and only then upscaled through the normal image-resize/VAE-encode path.
- If both `参考图像` and `输入潜空间` are connected, `参考图像` remains the reference source; `输入潜空间` is decoded with the VAE first, then scaled in pixel space and VAE-encoded back into the target latent canvas.
...
- `输入潜空间` should generally be clean unless you intentionally use the no-reference fallback to finish a noisy advanced segment at original size before decoding it into the reference image.
...
`L13 参考重绘放大参数（高级）` is a separate settings node for the advanced version. It only contains structural controls such as target size, tile size, overlap, context, sample halo, blend mode, image scaling, tile order, preview frequency, and max tile count. It intentionally does not contain `降噪`, `重绘强度`, reference retention, adaptive subject denoise, or seam repair controls.

For a segmented run:

1. With a reference image: connect `参考图像`, optionally connect a clean `输入潜空间` as the decoded starting image, then run the chosen advanced segment.
2. Without a reference image: connect `输入潜空间`; the node first runs the selected advanced segment at the latent's original size, decodes that result into the reference image, then performs the high-resolution reference redraw.
