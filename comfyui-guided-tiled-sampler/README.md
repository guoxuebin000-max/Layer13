...
The main `L13 参考重绘放大` nodes keep `降噪` visible because it is the same denoise concept as KSampler img2img. Size/tile/detail controls are removed from the main node UI and use built-in defaults unless you connect `L13 参考重绘放大参数` to the `高级参数` input. Connected `高级参数` overrides custom width/height, tile size, overlap, context, detail perturbation, sample halo, blend mode, preview frequency, reference retention, subject denoise cap, background multiplier, and seam repair settings.
...
Unlike the normal node, the advanced node keeps the sampler denoise timeline at `1.0`, matching KSampler Advanced behavior. `起始步` / `结束步` define the actual sigma segment. The visible `降噪` value is used as the local masked redraw strength, so it scales the tile mask/write intensity and subject protection behavior instead of changing the sampler's step range.

If `输入潜空间` is connected, the first stage starts from that latent instead of re-encoding the reference image. This is the intended path for chaining leftover-noise segments. Keep `参考图像` connected as the ratio/target reference and visual anchor.

For a segmented run:

1. First segment: `加噪 = 启用`, `起始步 = 0`, `结束步 = 3`, `保留剩余噪声 = 启用`.
2. Next segment: connect the previous segment's latent to `输入潜空间`, keep the same `总步数` / `采样器` / `调度器` / prompt, set `加噪 = 禁用`, `起始步 = 3`, `结束步 = 总步数`, `保留剩余噪声 = 禁用`.
