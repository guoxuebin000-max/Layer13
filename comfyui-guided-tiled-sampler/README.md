...
If `输入潜空间` is connected, the first stage starts from that latent instead of re-encoding the reference image. This is only valid when the input latent is already the target latent size. Noisy leftover latents cannot be spatially upscaled and then continued; that destroys the noise/sigma distribution. For this reason, the advanced node requires `递进放大模式 = 关闭`.
